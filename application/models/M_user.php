...
class M_user extends CI_Model{
    ...
    private $secoundtable = 'table_user_party';
    ...
    public function getUserParty(){
      $a = $this->table;
      $b = $this->secoundtable;
      $this->db->select($a.'.*', $b.'.user_id', $b.'.party_id');
      $query = $this->db->join($b, $b.'.user_id = '.$a.'.id', 'right')->get($a);
      return $query->result();
    }
    ...
    public function getKtp(){
      $this->db->select('a.id, a.first_name, a.last_name, b.identity_card_number');
      $this->db->from('table_userprofiles b');
      $this->db->join('table_users a','b.user_id=a.id','left');
      $query = $this->db->get();
      if ($query->num_rows() > 0){
          return $query->result();
      }
