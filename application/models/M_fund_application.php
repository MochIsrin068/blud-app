...
class M_fund_application extends CI_Model{
    ...
    public function fetch($data){
      ...
      if($join!=null && is_array($join)){
        foreach($join as $j){
          // if($j['id'] == 'table_users.id'){
          //   $this->db->join(
          //     $j['table'],
          //     $j['id'].'='.$j['table'].'.user_id',
          //     $j['join']
          //   );
          // }else{
            $this->db->join(
    ...
    public function insertDoc($data){
      $this->db->insert('table_document_request', $data);
      return ($this->db->affected_rows() != 1) ? false : true;
    }

    public function getAkad($data){
      $query = $this->db->where($data)->get('table_document_request');
      return $query->result();
    }
