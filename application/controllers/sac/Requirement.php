...
class Requirement extends Sac_Controller {

    private $services = null;
    private $name = null;
    private $parent_page = 'sac';
    private $current_page = 'sac/requirement';
    private $form_data = null;

    public function __construct(){
        parent::__construct();
        $this->load->helper(array('form', 'url'));
        $this->load->library('services/Requirement_services');
        $this->services = new Requirement_services;
        $this->name = $this->services->name;
        $this->form_data = $this->services->form_data();
        $this->load->model(array('m_sac_requirement'));
    }
    ...
    public function index(){
        ...
        $pagination['total_records'] = (isset($key)) ? $this->m_sac_requirement->search_count($key, $this->name) : $this->m_sac_requirement->get_total();
        ...
        $for_table = $this->m_sac_requirement->fetch($fetch);
        ...
        $this->render( "sac/requirement/content");
    }
    ...
    public function detail($id=null){
      ...
      if($memberId !== false){
        ...
        $members = $this->m_sac_requirement->fetchMember($fetch3, $tbl);

        $tblDocument = 'table_document';
        $usersId = $this->input->post('idUser');
        $sd['user_id'] = $usersId;
        $memberDocument = $this->m_sac_requirement->getDocument($sd, $tblDocument);
      }
      ...
      $w['id'] = $id;
      $form_value = $this->m_sac_requirement->getWhere($w);
      if($form_value==false){
        redirect($this->current_page);
      }else{
      ...
      $fetch['order'] = array("field"=>"id","type"=>"ASC");
      $p['id'] = $id;
      $partyV = $this->m_sac_requirement->getWhere($p);
      foreach($partyV as $pid){
        $fetch['where'] = array("table_party.id"=> $pid->party_id);
      }
      $myTable = 'table_user_party';
      $member = $this->m_sac_requirement->fetchMember($fetch, $myTable);
      ...
      $this->render("sac/requirement/detail");
    }
    ...
    //   $insert = $this->m_sac_requirement->tolak($id, $data);
    //   return $insert;
    // }
    ...
    public function update($id, $data) {
      $insert = $this->m_sac_requirement->update($id, $data);
      return $insert;
    }
}
